...
/**
 * As long as Maven controls the BuildPomXMLFilter, the entities that need merging are known. All others can simply be
 * copied from source to target to restore the locationTracker
 *
    ...
    extends MavenMerger
{
    ...
    @Override
    protected void mergeBuild_Extensions( Build.Builder builder, Build target, Build source, boolean sourceDominant,
    ...
    @Override
    protected void mergeCiManagement_Notifiers( CiManagement.Builder builder, CiManagement target, CiManagement source,
                                                boolean sourceDominant, Map<Object, Object> context )
    {
        // don't merge
    }
                                                           ...
                                                           boolean sourceDominant, Map<Object, Object> context )
    {
        ...
        builder.dependencies( target.getDependencies().stream().map( d -> mergeDependency( d, sourceIterator.next(),
                                                                                           sourceDominant,
                                        ...
                                        Map<Object, Object> context )
    {
        ...
        builder.profiles( target.getProfiles().stream().map( d -> mergeProfile( d, sourceIterator.next(),
                                                                                sourceDominant, context ) ).collect(
                                                                                                                     Collectors.toList() ) );
    }

    @Override
    protected void mergeModelBase_Dependencies( ModelBase.Builder builder, ModelBase target, ModelBase source,
                                                boolean sourceDominant, Map<Object, Object> context )
    {
        Iterator<Dependency> sourceIterator = source.getDependencies().iterator();
        builder.dependencies( target.getDependencies().stream().map( d -> mergeDependency( d, sourceIterator.next(),
    ...
    @Override
    protected void mergePlugin_Dependencies( Plugin.Builder builder, Plugin target, Plugin source,
                                             boolean sourceDominant, Map<Object, Object> context )
    {
        Iterator<Dependency> sourceIterator = source.getDependencies().iterator();
        builder.dependencies( target.getDependencies().stream().map( d -> mergeDependency( d, sourceIterator.next(),
