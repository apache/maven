...
public class DefaultLog implements Log {
    ...
    /**
     * Wraps a logging call with Log API metadata: when build report capture is
     * active, sets the ThreadLocal with source class name, caller method name,
     * and thread ID, executes the actual SLF4J call, and clears the ThreadLocal.
     * <p>
     ...
     * <p>
     * When build report capture is not active, the logAction is invoked
     * directly with zero ThreadLocal cost.
     */
    private void withMetadata(Runnable logAction) {
        if (ProjectBuildLogAppender.hasReportCapture()) {
            // Only pay the StackWalker + ThreadLocal cost when metadata is needed
