...
public class CommonsCliLogOptions extends CommonsCliOptions implements LogOptions {
    ...
    public Optional<String> module() {
        if (commandLine.hasOption(CLIManager.MODULE)) {
            return Optional.of(commandLine.getOptionValue(CLIManager.MODULE));
        }
        return Optional.empty();
    }

    @Override
    public Optional<String> mojo() {
        if (commandLine.hasOption(CLIManager.MOJO)) {
            return Optional.of(commandLine.getOptionValue(CLIManager.MOJO));
        }
        return Optional.empty();
    }
    ...
    public Optional<String> level() {
        if (commandLine.hasOption(CLIManager.LEVEL)) {
    ...
    @Override
    public Optional<String> grep() {
        if (commandLine.hasOption(CLIManager.GREP)) {
            return Optional.of(commandLine.getOptionValue(CLIManager.GREP));
        }
        return Optional.empty();
    }
    ...
    public void displayHelp(ParserRequest request, Consumer<String> printStream) {
        ...
        printStream.accept("");
        printStream.accept("Filters:");
        printStream.accept("  mvnlog --module api               Show only modules matching 'api'");
        printStream.accept("  mvnlog --level WARN               Show only WARN and ERROR log events");
        printStream.accept("  mvnlog --grep 'deprecated'        Search log messages for 'deprecated'");
        printStream.accept("  mvnlog --mojo compile             Show only 'compile' mojo executions");
        printStream.accept("  mvnlog --module api --level WARN  Combine filters (all must match)");
        printStream.accept("");
        printStream.accept("JSON output:");
        printStream.accept("  mvnlog --json                     Output full JSON report");
    ...
    protected static class CLIManager extends CommonsCliOptions.CLIManager {
        ...
        protected void prepareOptions(org.apache.commons.cli.Options options) {
            ...
            options.addOption(Option.builder(JSON)
                    ...
                    .get());
            options.addOption(Option.builder(MODULE)
                    .longOpt("module")
                    .hasArg()
                    .argName("pattern")
                    .desc("Filter to modules whose artifactId contains <pattern> (case-insensitive)")
                    .get());
            options.addOption(Option.builder(MOJO)
                    .longOpt("mojo")
                    .hasArg()
                    .argName("pattern")
                    .desc("Filter to mojos whose goal contains <pattern> (case-insensitive)")
                    .get());
            options.addOption(Option.builder(LEVEL)
                    .longOpt("level")
                    .hasArg()
                    .argName("LEVEL")
                    .desc("Filter log events by minimum level (TRACE, DEBUG, INFO, WARN, ERROR)")
                    .get());
            options.addOption(Option.builder(GREP)
                    .longOpt("grep")
                    .hasArg()
                    .argName("pattern")
