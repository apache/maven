...
public class CommonsCliUpgradeOptions extends CommonsCliOptions implements UpgradeOptions {
    ...
    public Optional<String> model() {
        if (commandLine.hasOption(CLIManager.MODEL)) {
            return Optional.of(commandLine.getOptionValue(CLIManager.MODEL));
        }
        return Optional.empty();
    }
    ...
    @Override
    public Optional<Boolean> fixModel() {
        if (commandLine.hasOption(CLIManager.FIX_MODEL)) {
            return Optional.of(Boolean.TRUE);
        }
        return Optional.empty();
    }
    ...
    public void displayHelp(ParserRequest request, Consumer<String> printStream) {
        ...
        printStream.accept("      --fix-model       Fix Maven 4 compatibility issues in POM files");
        ...
        printStream.accept("");
        printStream.accept("Default behavior: --fix-model and --plugins are applied if no other options are specified");
        printStream.accept("");
    }
    ...
    protected static class CLIManager extends CommonsCliOptions.CLIManager {
        ...
        public static final String FIX_MODEL = "fix-model";
        ...
        protected void prepareOptions(org.apache.commons.cli.Options options) {
            ...
            options.addOption(Option.builder(INFER)
                    ...
                    .build());
            options.addOption(Option.builder(FIX_MODEL)
                    .longOpt("fix-model")
            ...
            options.addOption(Option.builder(ALL)
                    .longOpt("all")
                    .desc("Apply all upgrades (equivalent to --model 4.1.0 --infer --fix-model --plugins)")
                    .build());
        }
    }
}
