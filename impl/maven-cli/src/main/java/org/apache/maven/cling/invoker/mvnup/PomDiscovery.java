...
public class PomDiscovery {
    ...
    /**
     * Discovers and loads all POM files starting from the given directory path string.
     *
     * @param startDirectoryPath the directory path string to start discovery from
     * @return a map of Path to Document for all discovered POM files
     * @throws IOException if there's an error reading files
     * @throws JDOMException if there's an error parsing XML
     */
    public static Map<Path, Document> discoverPoms(String startDirectoryPath) throws IOException, JDOMException {
        return discoverPoms(Paths.get(startDirectoryPath));
    }
    ...
    private static void discoverModules(Path currentDirectory, Document pomDocument, Map<Path, Document> pomMap)
        ...
        boolean is410OrLater = "4.1.0".equals(modelVersion) || isNewerThan410(modelVersion);
