...
public class DefaultModelXmlFactory implements ModelXmlFactory {
    ...
    public Model read(@Nonnull XmlReaderRequest request) throws XmlReaderException {
        ...
        boolean isInvalidNamespace = !"http://maven.apache.org/POM/4.0.0".equals(model.getNamespaceUri());
        if (isInvalidNamespace) {
            throw new XmlReaderException(
                    "Invalid namespace '" + model.getNamespaceUri()
                            + "'. Only 'http://maven.apache.org/POM/4.0.0' is supported.",
                    null,
                    null);
        }
        return model;
