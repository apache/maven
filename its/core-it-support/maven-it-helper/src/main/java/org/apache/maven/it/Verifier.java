...
public class Verifier {
    ...
    public Path getArtifactPath(String gid, String aid, String version, String ext, String classifier) {
        ...
        return getLocalRepository().resolve(executorTool.artifactPath(executorRequest(), gav, null));
    }
    ...
    public Path getArtifactMetadataPath(String gid, String aid, String version, String filename, String repoId) {
        ...
        gav += filename;
        return getLocalRepository().resolve(executorTool.metadataPath(executorRequest(), gav, repoId));
    }
        ...
        if (filePath.contains("!/")) {
            ...
            if (filePath.indexOf('*') > -1) {
                ...
                try {
                    ...
                    exists = Files.exists(expectedPath);
                    resolvedPath = expectedPath.toString();
                } catch (InvalidPathException e) {
                    // Fall back to File API for paths that NIO2 rejects (e.g., trailing spaces on Windows)
                    File expectedFile = new File(basedir.toFile(), filePath);
                    exists = expectedFile.exists();
                    resolvedPath = expectedFile.getPath();
                }

                if (!exists) {
                    if (wanted) {
                        throw new VerificationException("Expected file was not found: " + resolvedPath);
                    }
                } else {
                    if (!wanted) {
                        throw new VerificationException("Unwanted file was found: " + resolvedPath);
                    }
                }
            }
        }
