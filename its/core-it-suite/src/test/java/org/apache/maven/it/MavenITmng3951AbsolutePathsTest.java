...
public class MavenITmng3951AbsolutePathsTest extends AbstractMavenIntegrationTestCase {
    ...
    public void testitMNG3951() throws Exception {
        ...
        // Only strip the root on Windows where this creates a drive-relative path.
        // On Unix, Path.subpath() creates a plain relative path that Maven resolves
        // from CWD rather than from the filesystem root, unlike the old File API.
        if (!repoDir.toString().startsWith("/") && getRoot(repoDir).equals(getRoot(testDir))) {
            verifier.addCliArgument(
                    "-Dmaven.repo.local=" + File.separator + repoDir.subpath(0, repoDir.getNameCount()));
        }
