...
public class MavenITmng3503Xpp3ShadingTest extends AbstractMavenIntegrationTestCase {
    ...
    public void testitMNG3503NoLinkageErrors() throws Exception {
        Path dir = extractResourcesAsPath("/mng-3503/mng-3503-xpp3Shading-pu11");

        // First, build the test plugin
        Verifier verifier = newVerifier(dir.resolve("maven-it-plugin-plexus-utils-11").getAbsolutePath());
        ...
        verifier = newVerifier(dir.toString());
        ...
        assertEquals("<root />", Files.readString(dir.resolve("target/serialized.xml").toPath()));
    }

    @Test
    public void testitMNG3503Xpp3Shading() throws Exception {
        Path dir = extractResourcesAsPath("/mng-3503/mng-3503-xpp3Shading-pu-new");

        // First, build the test plugin
        Verifier verifier = newVerifier(dir.resolve("maven-it-plugin-plexus-utils-new").getAbsolutePath());
        verifier.setAutoclean(false);
        verifier.deleteDirectory("target");
        verifier.addCliArgument("install");
        verifier.execute();
